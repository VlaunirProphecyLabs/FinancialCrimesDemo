...
def main():
    ...
    spark.conf.set("prophecy.metadata.pipeline.uri", "pipelines/pipe_financial_crimes_final")
    registerUDFs(spark)
    
    MetricsCollector.instrument(spark = spark, pipelineId = "pipelines/pipe_financial_crimes_final", config = Config)(
        pipeline
    )
